...
This package uses cgo to call functions in the C decNumber package.
    ...
    decQuad is only 128 bits, Context are also sent and received by value because they are small struct (28 bytes). Sane for strings, which are small arrays embedded in struct.
